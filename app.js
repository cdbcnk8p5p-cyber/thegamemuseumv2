const STORE='theGameMuseumV352', OLD='theGameMuseumV35', LEGACY='theGameMuseumV34', THEME='museumThemeV35';
const V35_MEDIA={
 'GM-0161':{image:'./assets/covers/charlie-and-the-chocolate-factory-ps2.jpg',archiveImage:'./assets/archive/charlie-and-the-chocolate-factory-ps2-original.jpg',price:2,shop:'CEX',date:'2026-08-05',notes:'Bought from CEX for £2. Clean official cover used for the museum display; original purchase photo preserved in the archive.'},
...
function applyV35(s){let seed=window.MUSEUM_SEED?.games||[],existing=s.games||[],ids=new Set(existing.map(g=>g.id)),missing=seed.filter(g=>!ids.has(g.id));s.games=[...missing,...existing].map(g=>{let u=V35_MEDIA[g.id];return u?{...g,...u,notes:u.notes||g.notes}:g});return s}
...
function dashboard(){
 ...
 let latest=g[0];if(latest){let art=artwork(latest),latestCover=$('#latestCover');latestCover.innerHTML=art?`<img src="${esc(art)}" alt="${esc(latest.title)} cover art" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="latest-fallback" style="display:none">${esc(initials(latest.title))}</span>`:`<span class="latest-fallback">${esc(initials(latest.title))}</span>`;$('#latestTitle').textContent=latest.title;$('#latestMeta').textContent=`${latest.platform} • ${latest.edition||'Standard'}${latest.price!=null?' • '+money(latest.price):''}`;$('#latestOpen').onclick=()=>openGame(latest.id)}
}
...
$('#exportBtn').onclick=()=>download(`the-game-museum-v3.5.3-${new Date().toISOString().slice(0,10)}.json`,'application/json',JSON.stringify(state,null,2));
