...
function dashboard(){
 ...
 $('#progressPercent').textContent=pct+'%';$('#progressBar').style.width=pct+'%';$('#progressText').textContent=`${g.length} app catalogue entries loaded. The confirmed full museum total remains ${state.museumTarget||g.length} while the catalogue audit continues.`;
...
function render(){platformFilter();dashboard();collection();wishlist();cex();stats();timeline()}
